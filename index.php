...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($errors['name']) {
        ...
        // Выводим сообщение.
        $messages[] = '<div class="error">Ошибка в имени.</div>';
    }
    if ($errors['phone']) {
        setcookie('phone_error', '', 100000);
        $messages[] = '<div class="error">Ошибка в поле телефона.</div>';
    }
    if ($errors['email']) {
    ...
    if ($errors['languages']) {
        ...
        $messages[] = '<div class="error">Заполните любимые языки программирования.</div>';
    }
    if ($errors['bio']) {
